...
def test_given_order_and_alternative_order_when_pruning_then_return_equal_outputs(dummy_sample):
    order_gt = [2, 1, 3, 0]
    ...
    A_gt = model.prune(dummy_sample.to_numpy(), full_DAG(order_gt))

    model.order_ = order_equivalent
    A_equivalent = model.prune(dummy_sample.to_numpy(), full_DAG(order_equivalent))
