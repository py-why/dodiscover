...
def test_given_order_when_applying_full_DAG_then_fully_connected_adjacency_is_correct(dummy_dense):
    """
    Test full_DAG() function mapping an order to its unique
    adjacency matrix representation
    """
    order = [2, 1, 3, 0]
    assert np.allclose(full_DAG(order), dummy_dense)
...
def test_given_dataset_when_fitting_the_hessian_then_hessian_is_symmetric(dummy_sample):
    ...
    stein = SteinMixin()
    H = stein.hessian(dummy_sample.to_numpy(), 0.001, 0.001)
